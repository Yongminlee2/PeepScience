...
StageData _trivialStage(String id) => StageData.fromJson(jsonDecode(
    '{"id":"$id","world":1,"index":1,'
    '"goal":{"type":"ball_in_basket"},'
    '"preset":[{"type":"basket","x":8,"y":6,"angle":0}],'
    '"tray":[{"type":"rubber_ball","count":1}],'
    '"solution":[{"type":"rubber_ball","x":8,"y":2,"angle":0}]}') as Map<String, dynamic>);
...
void main() {
  ...
  test('StageLoader: 정상 JSON은 StageData로, 깨진 JSON은 StageLoadError로', () {
    const validJson = '{"id":"t","world":1,"index":1,'
    ...
    expect(() => StageLoader.parse('bad', '{not valid json'),
        throwsA(isA<StageLoadError>()));
    expect(() => StageLoader.parse('bad', '{"id":"bad"}'),
        throwsA(isA<StageLoadError>()));
  });

  testWidgets(
      '클리어 콜백이 발생하면 progress에 기록되고, 다음 버튼을 누르면 다음 스테이지 화면으로 교체된다',
      (t) async {
    ...
    final game =
        t.widget<GameWidget<PiyakGame>>(find.byType(GameWidget<PiyakGame>))
            .game!;
    ...
    expect((await progress.cleared()).contains('w1_s01'), isTrue,
